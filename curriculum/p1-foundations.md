...
每日目標是 30–60 分鐘，但影片長度不等於實際練習時間。零基礎跟做時，20–30 分鐘教材花掉 45–90 分鐘很正常。超過當日額度就停在可恢復的位置，下次接續；不要為了 Day number 熬夜趕進度。
...
- 確認目前使用的是 Blender Stable build，將精確版本記入 STATUS。
...
必須至少使用 Edit Mode、Extrude、Loop Cut 或 Inset、Bevel 或 Modifier。允許查「某功能怎麼按」，不允許查「如何做這個物件」。
...
### Day 13 — Camera、Render 與第一張作品照
學習：Camera、構圖、焦距基本概念、Render。

Presentation Quest：挑 Day 6–12 中最喜歡的一個作品，重新整理材質與燈光，架好 Camera，算出一張你願意保存的作品圖。

這一天刻意提供早期成就感：前兩週作品可以很簡單，但要第一次體驗「我用 Blender 做出一張完整畫面」。漂亮不是通關條件。

### Day 14 — 新手村 Boss
從零做一個以前沒有跟著教學做過的小物件。

限制：不看完整作品教學；可以查 Blender 功能文件；必須自行決定 primitive、建模流程、材質、燈光與相機。

通關條件不是「漂亮」，而是能解釋自己為什麼使用這些工具。

## Day 15–30：第一個整合專案

優先使用 Blender Guru Donut 的目前 Stable Blender 對應版本；若教材版本與已安裝 Stable build 明顯不相容，由教練選擇同級 beginner project 並在 STATUS 記錄理由。

Donut 在這裡不是第一次介紹 Blender 操作，而是把已學過的 Modeling / Modifier / Material / Lighting / Camera 串成完整 workflow。

每完成一個新概念，下一個任務優先安排不同物件的遷移練習，而不是單純把同一段作品再背一次。

## Day 31–45：獨立小作品

逐步加入精確尺寸、Origin / Pivot、Apply Transform、Snapping、更多 Modifier、UV 基礎。至少完成 3 個不同類型的小物件，避免只熟悉單一 tutorial workflow。

## Day 46–52：UV / Texture 基礎

學習 seam、unwrap、UV island、簡單 texture workflow。重點是為 P2 與角色製作建立概念，不追求專業貼圖師深度。
