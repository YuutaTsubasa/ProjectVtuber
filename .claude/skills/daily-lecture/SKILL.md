...
| # | Section | 內容 |
|---|---|---|
| 01 | 今天做什麼 | 逐字沿用 curriculum 的 Quest；無遷移挑戰就刪掉那張 warn 卡 |
| 02 | 為什麼要學這個 | 先講解決什麼問題。此節禁止出現快捷鍵表 |
| 03 | 動手前要知道的 | **只留 1–2 張**動手前非知道不可的概念卡 |
| 04 | 操作流程 | 5–9 步，主線只寫目標＋提示，按鍵鎖在 answer 摺疊 |
| 05 | 卡住了？ | 3–5 個症狀，第一層自我診斷，第二層才是答案 |
| 06 | 常見錯誤 | 常見做法 / 為什麼有問題 / 改成 |
| 07 | 驗收標準 | 有原文就逐字沿用；沒有就依「驗收標準的來源」推導並標示 |
| 08 | 完成後回報 | 3 題，至少一題問「為什麼這樣選」 |
| 09 | 快捷鍵速查 | 三欄；「解決什麼問題」欄不可留空或寫「同左」 |

若某一天真的沒有第 06 節的素材，仍保留節標題並寫出至少一列真實會踩的錯，不要留空表格。

### 這個順序是怎麼來的

原本的順序是「為什麼 → 概念 → 快捷鍵 → Quest → 流程」，實測 Day 1 要讀 2690 字（全頁 42%）才碰得到第一個能動手的步驟。使用者回報「很不新手向，從上面看下來跳來跳去」。三個具體成因：

1. **快捷鍵表是查表用的，卻放在閱讀位置。** 還沒按過任何一個鍵就被要求看完十幾個，記不住，到了流程還得再看一次——同一份資訊出現兩次、兩次都不完整。**所以它降到第 09 節當附錄。**
2. **「卡住了」排在驗收之後，但需要它的時間點在流程當中。** 卡住要往下捲過驗收去找症狀再捲回來。**所以它緊貼在流程後面。**
3. **概念出現在你有東西可以對照之前，只能硬背。** **所以第 03 節只留貫穿全天的那 1–2 張**，其餘搬進會用到它的那一步。

重排後動手前的份量降到全頁 13–20%。改這個順序前先讀這一段，不要改回去。

### 概念卡放哪裡

- **貫穿全天、動手前非知道不可** → 留在第 03 節。例如 Day 1 的「選取決定了作用對象」、Day 3 的「同一顆 Cube，兩種改」。
- **只在某一步會用到** → 搬進那一步，放在 `.hint` 之後、`details.answer` 之前，寫成 `<details><summary>{概念標題}（選讀）</summary>`。例如 Day 1 的「三個軸與正交視角」屬於「練習切換正交視角」那一步。

判準是「讀者讀到它的時候，手上有沒有東西可以對照」。

## 驗收標準的來源

curriculum 只有少數日子寫了 `Acceptance Criteria`——P1 的 14 天裡只有 Day 1 有，Day 4／14 是「通關條件」、Day 6 是「回報」，其餘十天什麼都沒有。但第 07 節是不可刪的契約，所以多數日子必須推導。沒有規則的話，每一天的標準都會不一樣。
...
3. **存檔路徑固定算一條**，不視為加碼。第 08 節的自檢指令需要它。
...
| `{{CRITERIA_SOURCE}}` | `Quest 與通關條件`（只在第 07 節的標示行用到，見「驗收標準的來源」規則 5） |
...
| 快捷鍵表放進第 02 節 | 違反「先問題後按鍵」，也把附錄搬回閱讀位置 | 第 02 節只講問題情境，按鍵表留在第 09 節 |
