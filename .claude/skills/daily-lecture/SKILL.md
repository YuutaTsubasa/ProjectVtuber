...
5. **推導出來的要標示，而且要講對來源。** 在 `<ul class="check">` 前加一行，把 `{{CRITERIA_SOURCE}}` 換成當日<strong>實際用到</strong>的來源：
   ...
   模板裡那段給產生者看的說明註解，**兩條路徑都要在交付前刪掉**，不要留進成品——它對學習者沒有意義，而且取消註解之後內容就過期了。它不帶大括號，`grep "{{"` 抓不到。
...
上面那道只掃得到大括號。模板裡給產生者看的說明註解不帶大括號，會整段漏過去，所以另外掃一次：

```bash
grep -o "CRITERIA_SOURCE" lectures/p1/day01.html | wc -l
```

同樣必須是 0。非 0 代表那段說明註解被留進成品了。
